    ...
    if (!error)
    {
      ...
      else if (status == 6)
      {
        ...
        else if (a == 2)
        {
          if (b == 1)
          {
            roboclaw1.ForwardM2(address3, 100);
          }
          else if (b == 0)
          {
...
int rotationWithPID(int _angle)
{
  ...
  float kDifRotation = 25, kIntRotation = 0.5, kPrRotation = 70;
  float _dt = 1;
  int controlAct = 0;
  int logic = 0;
...
int move(int angle, long int length)
{
  ...
  while (logic == 0)
  {
    ...
    float fin = min(1.0, (float)(lengthOfRobot + 100) / (float)(1000));
    float st = min(1.0, (float)(length - lengthOfRobot) / (float)(300));
    roboclaw1.SpeedM1(address1, (int)(Va_base * fin * st));
    roboclaw1.SpeedM1(address2, (int)(Vb_base * fin * st));

...
def perspectiveTransform(src, saved):
    global loaded, loadedH,loadedMask
    ...
    def get_plan_view(src, dst, H=None, flag = False):
    ...
    def merge_views(src, dst, H=None, flag = True):
        plan_view, H, mask = get_plan_view(src, dst, H, flag)
    ...
    if saved:
        ...
        mask = loadedMask
        dst = cv2.imread(dst_path, -1)
        merge, H, mask = merge_views(src, dst, H, False)
        image = merge


    return image, H, mask

def save_file(np_array, np_array2, filename="data.txt",filename2="data2.txt"):
    with open(filename, 'w') as f:
        np.savetxt(f, np_array)
    with open(filename2, 'w') as f:
        np.savetxt(f, np_array2)

def load_file(filename="data.txt",filename2="data2.txt"):
    global loaded, loadedH,loadedMask
